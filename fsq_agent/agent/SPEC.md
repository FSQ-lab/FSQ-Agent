...
- `tools`: Builds OpenAI Agents SDK tools/MCP servers, harnesses, platform action tools, and exposes diagnostic capabilities.
...
- `_openai_runtime.py`: OpenAI Agents SDK client/provider setup, harness setup/teardown invocation, platform action tool construction, agent construction, MCP context management for non-platform integrations, MCP validation diagnostic step injection, `Runner.run_streamed` invocation, and SDK stream event mapping.
...
Android harness runs fail before backend startup when required process environment is missing or invalid. `ANDROID_HOME` must point to an existing Android SDK directory, and the environment variable named by `harness.platform.capabilities_config_env` must point to an existing Appium capabilities JSON file.

Goal pre-planning raises configuration errors when OpenAI Agents SDK configuration is unavailable and planning errors when the SDK does not return a valid `GoalPrePlan`. Pre-planning does not open MCP servers, create a harness, run harness setup, call UI/external action tools, or generate reports. Read-only knowledge lookup tool failures are surfaced to the planner as warnings so it can continue when possible.

During `FsqAgent.run`, a task with no ordered key actions is treated as a goal-only task. Before harness setup or external UI actions begin, the orchestrator runs the existing goal pre-planner with the task goal, converts returned `GoalKeyAction` values into `Task.key_actions`, emits planning events on the same run timeline, and then executes the normal task runtime. If the pre-plan returns no key actions, execution still proceeds with the original goal-only task and a warning event; if pre-planning raises an error, the task run fails before external actions are attempted.
...
- The configured harness owns deterministic run/case setup and teardown before and after the model tool loop. Those calls are not agent reasoning steps; they are runtime preparation/cleanup and are emitted as harness lifecycle events. Platform operations used by those hooks come through the harness and its platform adapter.
- The agent must create a pre-plan before external actions, derive task success criteria from the description, private knowledge, and skills when the user did not provide criteria, include those criteria in the pre-plan, execute through harness-supplied platform actions, configured local tools, optional non-platform MCP tools, and skills, and adapt the plan when tool feedback changes the best path.
...
- Task runs emit live `RunEvent` values for harness lifecycle, platform actions, planning summaries, MCP tool listing, SDK tool calls, tool outputs, failures, and completion. Events are for user-visible progress and must not expose hidden model chain-of-thought. Runtime interruptions such as debugger cancellation or keyboard interrupt emit a final `run_failed` event before being re-raised.
...
- Platform-specific action selection, argument rules, and recovery recipes belong in configured skill Markdown rather than agent runtime branches. The agent consumes those skills as current runtime policy while backend transports remain hidden behind harness/platform adapters. Harness-supplied platform action schemas should be specific enough for the agent to perform common UI work in one tool call when possible, including precise locator arguments and direct element id reuse, instead of requiring repeated model-level find/tap/retry loops for backend operations that can be composed inside the adapter.
...
- SDK stream events and harness platform action events are mapped into `RunEvent` values that preserve real tool or platform action names, call IDs when present, redacted arguments, output previews, errors, timing, and origin when known. Reports reconstruct real tool and platform action calls from these events rather than treating plan or runner summary records as tools.
...
- Final result judgment is performed by a separate evidence-based verification agent task after the main automation run. The verification task has no MCP servers, no harness, no external action tools, and no image inputs; it receives the authoritative task/case intent, the main agent's structured claims when available, execution records, normalized event/tool-call/platform-action records, and persisted artifact excerpts. The verification agent must decide success, failure, or inconclusive from supplied execution evidence only.
