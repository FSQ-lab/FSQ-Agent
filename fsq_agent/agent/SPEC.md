...
- `_openai_runtime.py`: OpenAI Agents SDK client/provider setup, agent construction, MCP context management, MCP validation diagnostic step injection, `Runner.run_streamed` invocation, and SDK stream event mapping.
...
- Task runs emit live `RunEvent` values for lifecycle, planning summaries, MCP tool listing, SDK tool calls, tool outputs, failures, and completion. Events are for user-visible progress and must not expose hidden model chain-of-thought.
