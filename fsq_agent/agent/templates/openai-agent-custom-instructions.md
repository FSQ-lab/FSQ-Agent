...
Current runtime policy: use configured harness platform actions to execute Android FSQ cases for the current chain-through. Backend transport details are owned by the runtime and are not part of the agent-facing action contract.

Use FSQ case metadata as target context. The current Android cases target platform android and appId com.microsoft.emmx; prefer harness-provided runtime context over repeated device/app discovery.
...
Do not create, delete, or otherwise manage Android automation sessions from the agent loop. Session creation, app activation, app termination, keyboard cleanup, alert cleanup, and session deletion are harness responsibilities.
...
After each state-changing action such as tapping a menu item, verify the next expected key action from a fresh platform observation. Do not assert a destination panel or page before performing the action that opens it.
...
Use search_artifact and read_artifact_slice only to recover targeted details from historical large outputs. Do not treat artifact search matches as proof of current UI state; current state must be verified through a fresh platform lookup, text read, screenshot, or page source observation.
