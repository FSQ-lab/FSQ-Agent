...
- `StrictCoreEvidenceSettings`: Pydantic model for default strict-core evidence capture policy. It contains `capture_before`, `capture_after`, `capture_on_failure`, and `artifact_kinds`, and can be converted into the shared `EvidencePolicy` model used by executable steps.
- `StrictCoreHarnessSettings`: Pydantic model for deterministic strict-core harness pacing and default evidence capture. `step_interval_seconds` defaults to `1.0`, must be non-negative, and is passed to `StepSequenceRunner` by entry-layer strict execution. `evidence` defaults to before/after/failure screenshot capture for platform-backed FSQ steps. Pacing is timing-only and must not be serialized into FSQ commands or evidence as a synthetic step.
