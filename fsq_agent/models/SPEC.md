...
- `StrictCoreSettings`: Pydantic model for deterministic strict-core execution policy owned by configuration. It contains `step_interval_seconds`, a non-negative float controlling the pacing sleep before each subsequent executed strict step. The default is `1.0`; `0` disables this implicit pacing.
...
- Strict-core pacing is model-owned as `StrictCoreSettings.step_interval_seconds` so CLI/config can control deterministic inter-step sleeps without encoding timing policy inside FSQ YAML, reports, or platform harnesses.
