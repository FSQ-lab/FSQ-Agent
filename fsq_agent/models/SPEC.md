...
- `capture_evidence` on capability metadata is a typed cross-module contract for reviewed platform tools whose dynamic execution should receive the standard before/after screenshot and UI-tree evidence policy; the flag defaults to `False` and is not user YAML configuration.
