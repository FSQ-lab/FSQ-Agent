...
- `CapabilityDefinition`: Pydantic model describing one executable capability. It contains canonical `name`, accepted `aliases`, `executor_kind` (`common`, `harness`, or `driver`), parameter model/schema identity, `step_kind`, description, optional platform/backend/owner metadata, `capture_evidence`, optional `post_action_delay_seconds`, `sensitivity`, optional `ReplayPolicy`, and safe provenance metadata. It is serializable metadata only and must not hold SDK tool objects, driver instances, or function callables. `post_action_delay_seconds=None` means inherit the configured executor-kind default; `0` explicitly disables the runner-owned post-action delay for that capability; positive values override the configured default.
...
- `CommonToolCall`: Pydantic model describing one SDK-neutral common utility invocation request against a canonical capability name.
- `CommonToolResult`: Pydantic model describing a normalized common utility invocation response before it is wrapped as a `CapabilityExecutionResult` by `StepRunner`.
...
- Capability metadata is the authoritative executable contract. `CapabilityDefinition`, `ReplayPolicy`, post-action delay overrides, and registry snapshots replace separate CommonTool, harness function, and static Android action schema authority in the target architecture. Decorator declarations and platform catalog validation live in `capabilities`; `models` owns only the serializable contracts they produce.
