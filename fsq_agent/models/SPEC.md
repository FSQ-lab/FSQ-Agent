...
- `HarnessSettings`: Pydantic model selecting the platform harness configuration used by goal-driven task execution. It contains platform-specific harness settings, dynamic harness evidence capture defaults, and `strict_core` pacing/evidence settings owned by strict-core harness execution.
...
- `HarnessEvidenceSettings`: Pydantic model for default dynamic platform harness evidence capture policy. It contains `capture_before`, `capture_after`, `capture_on_failure`, and `artifact_kinds`, and can be converted into the shared `EvidencePolicy` model used by dynamic harness tool actions. Its default artifact list is empty so dynamic runs do not automatically capture screenshots unless configured.
- `StrictCoreEvidenceSettings`: Pydantic model for default strict-core evidence capture policy. It contains `capture_before`, `capture_after`, `capture_on_failure`, and `artifact_kinds`, and can be converted into the shared `EvidencePolicy` model used by strict executable steps. Its defaults request screenshot capture for strict replay auditability.
- `StrictCoreHarnessSettings`: Pydantic model for deterministic strict-core harness pacing and default strict-core evidence capture. `step_interval_seconds` defaults to `1.0`, must be non-negative, and is passed to `StepSequenceRunner` by entry-layer strict execution. `evidence` defaults to before/after/failure screenshot capture for platform-backed strict FSQ steps. Pacing is timing-only and must not be serialized into FSQ commands or evidence as a synthetic step.
