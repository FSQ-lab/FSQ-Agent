...
- `HarnessFunctionSchema`: Pydantic model describing one concrete platform harness or driver action that can be exposed as an OpenAI-compatible function schema. It contains the tool name, description, strict parameter JSON schema, platform, Python handler/driver method name, optional FSQ action provenance, reviewed evidence-capture intent through `capture_evidence`, and backend metadata. It is serializable only; it must not hold OpenAI Agents SDK runtime objects.
...
- `HarnessFunctionSchema` is deliberately serializable. It is the single platform action function-call schema source and does not import or wrap OpenAI Agents SDK tool objects. Harness-owned platform actions such as Android `assertWithAI` use the same schema path as driver-backed platform actions. Its `capture_evidence` flag is a typed cross-module contract for reviewed platform tools whose dynamic harness execution should receive the standard before/after screenshot and UI-tree evidence policy; the flag defaults to `False` and is not user YAML configuration.
