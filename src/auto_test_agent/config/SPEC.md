...
Load, merge, normalize, and validate runtime configuration for the OpenAI Agents SDK runtime, Azure OpenAI model deployment, MCP servers, CLI tools, automation skills, observation settings, and output directories.

## Dependencies

- `models`: Uses `AgentSettings`, `OpenAIAgentsSettings`, `MCPServerConfig`, `CLIToolConfig`, `ShellSettings`, `SkillConfig`, `ObservationSettings`, `OutputSettings`, and `ConfigurationError`.

## Public Interface

Current `__init__.py` exports via `__all__`:

- `Settings`: Runtime settings aggregate model that combines agent, OpenAI Agents SDK provider, MCP, CLI, shell, skills, observation, and output configuration.
